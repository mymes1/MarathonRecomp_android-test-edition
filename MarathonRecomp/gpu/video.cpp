...
bool Video::CreateHostDevice(const char *sdlVideoDriver, bool graphicsApiRetry)
{
    ...
    // exercises the BC fallback paths on hardware that natively supports BC (e.g. Adreno).
    {
        ...
        // Opposite override: force the native BC path even if BC was not reported, skipping
        // the ETC2/EAC transcode and its per-texture CPU overhead. Only correct on GPUs that
        // actually support BC (e.g. Xclipse) but end up on the fallback path anyway; on
        ...
        if (ImGui::CollapsingHeader("GPU", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (ImGui::TreeNode("Features"))
            {
                ...
                if (ImGui::BeginTable("Features", 2))
                {
                    ...
                    IMGUI_GENERIC_ROW("BC Textures", "%s", g_capabilities.textureCompressionBC
